...
- **Auto-merge advances the ref first, then the user's checkout when safe** — `merge_feature_into_base` still works in a `--detach`ed temp worktree and CAS-advances the ref via `update-ref OLD NEW`; that remains authoritative. On a clean merge `committer._advance_user_checkout_after_merge` then fast-forwards the primary checkout at `project_root` via `git read-tree -u -m <pre_sha> <new_sha>` (not `merge --ff-only` — HEAD is a symbolic ref and already points at `new_sha` once the CAS lands, so `merge` sees "Already up to date" and no-ops without refreshing index/worktree). For the same reason the safety gates compare the index and worktree to the *pre-merge base SHA* (`git diff --cached --quiet <pre_sha>` + `git diff --quiet <pre_sha>` + untracked check) — comparing to `HEAD` would misreport the phantom-reversion state as user dirt. The checkout advance is skipped (with a visible `checkout skipped: <reason>` suffix on the MERGED transition note) when (a) the checkout is on a different branch (or detached), (b) the index/worktree diverges from the pre-merge tree, or (c) untracked files would be clobbered. The ref has still advanced in every skip case, so the user just sees the existing "behind" state until they clean up. Gated off by `git.advance_user_checkout = false`.
