...
- **Auto-merge integrates via a detached temp worktree** — `merge_feature_into_base` always works in a `--detach`ed temp worktree and CAS-advances `refs/heads/<base>` via `update-ref OLD NEW`. The user's checkout at `project.root` is never used as a workspace for the merge itself. After a clean success the committer optionally fast-forwards the user's checkout to the new tip via `advance_user_checkout`, but only when (a) the checkout is on `base_branch`, (b) the working tree is clean, and (c) HEAD equals the pre-merge base sha — otherwise it silently leaves the checkout alone. Operators who deliberately park another branch at `project.root` opt out via `git.advance_user_checkout = false`.
