...
# `agent.model`. `stub` is deliberately excluded — it's a test fixture,
# never a user-toggled provider.
...
class AgentConfig:
    ...
    execute_prompt_template: str = (
        ...
        "refactor it.\n\n"
        "4. Scope guard: if you discover an out-of-scope issue, note it in "
        "the Follow-ups section of your agent-log rather than fixing inline. "
        "Stay focused on the approved plan — operators triage follow-ups "
        "into `docs/backlog/<slug>.md` during review.\n\n"
    ...
    # Max age (hours) for a persisted session_id / thread_id to still be
    ...
    # `--resume` round-trip. Generic knob — each `Provider` decides
    # whether to honour it (`StubProvider` returns `None` and the age
    # gate is skipped; `ClaudeProvider` / `CodexProvider` honour it).
